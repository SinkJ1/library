...
export class BookService {
  ...
  protected convertDateFromClient(book: IBook): IBook {
    return Object.assign({}, book, {
      dateOfIssue: book.dateOfIssue?.isValid() ? book.dateOfIssue.format(DATE_FORMAT) : undefined,
    });
  }

  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.dateOfIssue = res.body.dateOfIssue ? dayjs(res.body.dateOfIssue) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((book: IBook) => {
        book.dateOfIssue = book.dateOfIssue ? dayjs(book.dateOfIssue) : undefined;
      });
    }
    return res;
  }
